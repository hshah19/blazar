...
class PhysicalHostPlugin(base.BasePlugin, nova.NovaClientWrapper):
    ...
    def on_start(self, resource_id):
        ...
        for reservation in reservations:
            ...
            for allocation in db_api.host_allocation_get_all_by_values(
                ...
                # (hs634): we are not going to create or update host aggregates
                #pool.add_computehost(reservation['resource_id'],
                #                     host['service_name'])

        self._update_tenant_quota(operator.add)
    ...
    def on_end(self, resource_id):
        ...
        self._update_tenant_quota(operator.sub)

    def _update_tenant_quota(self, operation):
        project_id = None
        try:
            ctx = context.current()
            project_id = ctx.project_id
        except RuntimeError:
            e = manager_ex.ProjectIdNotFound()
            LOG.error(e.message)
            raise e
        else:
            try:
                current_quota_set = self.nova.quotas.get(project_id)
                flavor_list = self.nova.flavors.list()
                flavor = None
                if flavor_list is not None and len(flavor_list) > 0:
                    flavor = flavor_list[0]
                if current_quota_set is not None and flavor:
                    updated_cores = operation(current_quota_set.cores, flavor.vcpus)
                    updated_ram = operation(current_quota_set.ram, flavor.ram)
                    self.nova.quotas.update(project_id,cores=updated_cores, ram=updated_ram)
            except Exception, ex:
